  ...
  browser.tabs.query(queryInfo).then(tabs => {
    ...
    if (request.headers['x-fstrz']) {
      ...
      getFstrzCookie(request.details.url)
        .then(fstrzCookie => {
          if (
            (fstrzCookie && fstrzCookie.value === 'false') ||
            request.headers['x-fstrz'].indexOf('Z') >= 0
          ) {
            $('#fstrz-false').hide()
          } else {
            $('#fstrz-true').hide()
    ...
    if (request.pluggedToCDN()) {
      $('#cdn_status').html(
        request.servedByCDN()
          ? '<a class="btn btn-default">HIT</a>'
          : '<a class="btn btn-default">MISS</a>'
      )
    } else {
      $('#cdn_div').hide()
    }
    ...
    document
      .getElementById('feature-flag-table')
    ...
    browser.storage.local
      ...
      .then(res => {
        $('#disable-fasterize-cache').prop(
          'checked',
          res && res['disable-fasterize-cache']
        )
      })
      .catch(logError)
    ...
    $('#disable-fasterize-cache').change(function() {
      browser.storage.local
        .set({ 'disable-fasterize-cache': this.checked })
        .catch(logError)
      browser.runtime
        .sendMessage({
